...
class MainRepositoryImpl extends MainRepository {
  ...
  @override
  Future<Either<List<ListDataEntity>?, CloseUnloadingEntity?>> closeUnloading(
      String params) async {
    final result = await _remoteDataSource.closeUnloading(params);
    if (result != null && result.code == 200) {
      return right(result.toEntity());
    } else if (result!.code == 400) {
      List<ListDataEntity>? dataEntity = [];
      for (var items in result.listData!) {
        dataEntity.add(
          items.toEntity(),
        );
      }
      return left(dataEntity);
    } else {
      return left(null);
    }
  }
