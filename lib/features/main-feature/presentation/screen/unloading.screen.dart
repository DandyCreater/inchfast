  ...
  Widget build(BuildContext context) {
    return BlocListener<SubmitUnloadingBloc, SubmitUnloadingState>(
      listener: (context, state) {
        ...
        if (state is SubmitUnloadingSuccess) {
          ...
          HandlingWidget.successDialog(
              context: context,
              onOk: () {
                context.read<cubit.SubmitUnloadingCubit>().resetValue();
      ...
      child: Scaffold(
            ...
            BlocBuilder<UnloadingBloc, UnloadingState>(
              builder: (context, state) {
                ...
                if (state is UnloadingSuccess) {
                  ...
                  context.read<cubit.SubmitUnloadingCubit>().setIdFzNoComplete(
                        data?.id ?? '',
                      );
                  return Column(
                    children: [
                              ...
                              builder: (context, state) {
                                ...
                                if (state is UnloadingStatusSuccess) {
                                  context
                                      .read<cubit.SubmitUnloadingCubit>()
                                      .setStatus(
                                        int.tryParse(state.listValue.first.id ??
                                                "0") ??
                                            0,
                                      );
                                  return Padding(
        ...
        bottomNavigationBar: BlocBuilder<UnloadingBloc, UnloadingState>(
          builder: (context, state) {
            if (state is UnloadingSuccess) {
                ...
                return SafeArea(
                  child: Padding(
                    ...
                    child: UIButton.primaryFull(
                      ...
                      onTap: () {
                        var data =
                            context.read<cubit.SubmitUnloadingCubit>().state;
                        context
